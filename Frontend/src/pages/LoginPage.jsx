...
import AuthArtwork from '../components/site/AuthArtwork'
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 'calc(100vh - 60px)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 40px' }}>
        <div className="fade-in" style={{ width: '100%', maxWidth: 380 }}>
          ...
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.16em', color: '#7c5cf0', marginBottom: 8, textTransform: 'uppercase' }}>Welcome back</p>
          <h1 style={{ fontSize: 32, fontWeight: 900, color: '#fff', marginBottom: 36, letterSpacing: '-0.02em' }}>Sign in</h1>
            ...
            <Field label="Password" style={{ marginBottom: 28 }}>
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  placeholder="••••••••" className="noir-input" style={{ paddingRight: 44 }}
                />
          ...
          <p style={{ textAlign: 'center', marginTop: 24, fontSize: 14, color: '#888' }}>
            ...
            <Link to="/register" style={{ color: '#7c5cf0', fontWeight: 500 }}>Create one</Link>
          </p>
        </div>
      </div>

      {/* ── Right: artwork ── */}
      <div style={{ display: 'none' }} className="auth-artwork-panel">
        <AuthArtwork />
      </div>
      <style>{`
        @media (min-width: 768px) {
          .auth-artwork-panel { display: block !important; }
        }
      `}</style>
    </div>
  )

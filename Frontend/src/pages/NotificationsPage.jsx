...
export default function NotificationsPage() {
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '40px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {unreadCount > 0 && (
            <span style={{
              ...
              color: 'var(--brand)', padding: '2px 8px', borderRadius: 20,
            }}>
              {unreadCount} unread
            </span>
          )}
        ...
        {unreadCount > 0 && (
          <button
            ...
            style={{
              ...
              fontSize: 12, color: 'var(--brand)', background: 'var(--brand-tint)',
        ...
        {['all', 'unread'].map(f => (
          <button
            ...
            style={{
              ...
              background: readFilter === f ? 'var(--brand-soft)' : 'var(--glass-bg2)',
              color: readFilter === f ? 'var(--brand)' : 'var(--text-secondary)',
            }}
          >
            {f === 'all' ? 'All' : 'Unread'}
          </button>
        ...
        {CATEGORIES.map(c => (
          <button
            ...
            style={{
              ...
              cursor: 'pointer', transition: 'all 0.15s', border: 'none',
              background: filter === c ? 'var(--brand)' : 'var(--glass-bg2)',
              color: filter === c ? 'var(--brand-text)' : 'var(--text-muted)',
            }}
          >
            {CATEGORY_LABEL[c]}
          </button>
      ...
      {/* List */}
      <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--card-border)', borderRadius: 14, overflow: 'hidden' }}>
        {loading ? (
          ...
          displayed.map((n, i) => (
            <div
              ...
              style={{
                padding: '14px 18px',
                borderBottom: i < displayed.length - 1 ? '1px solid var(--card-border)' : 'none',
            ...
            >
              <div style={{
                ...
                boxShadow: n.isRead ? 'none' : '0 0 8px var(--brand-glow)',
              }} />
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                  <span style={{ fontSize: 14, fontWeight: n.isRead ? 400 : 600, color: n.isRead ? 'var(--text-secondary)' : 'var(--text-primary)' }}>
                    {n.title}
                  </span>
                  <span style={{ fontSize: 11, color: 'var(--text-muted)', flexShrink: 0 }}>{timeAgo(n.createdAt)}</span>
                </div>
                {n.body && (
                ...
                <div style={{ marginTop: 6, display: 'flex', gap: 8 }}>
                  {n.category && (
                    <span style={{
                      ...
                      color: 'var(--text-muted)', background: 'var(--glass-bg2)', padding: '2px 6px', borderRadius: 4,
                    }}>
                  ...
                  {!n.isRead && (
                    <button
                      ...
                      style={{
                        ...
                        color: 'var(--brand)', background: 'var(--brand-tint)', border: 'none',
